...
void SPI2_Init(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;	 //IO-->	PB12  NSS
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
    GPIO_Init(GPIOB, &GPIO_InitStructure);					 

    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;	 //IO-->PC6  RST					
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
    GPIO_Init(GPIOC, &GPIO_InitStructure);					 //根据设定参数初始化PF0、PF1
    GPIO_ResetBits(GPIOC,GPIO_Pin_6);			             //PC6输出低
    ...
    GPIO_SetBits(GPIOB, GPIO_Pin_13 | GPIO_Pin_14| GPIO_Pin_15); //PB5/6/7上拉
...
unsigned char ReadID(void)								//读SPI2对应的卡
{
    u8 i;
...
{
    u8 zhouzhiwei[] = {"4CFB24D9"};
    u8 yinzhibo[] = {"A1BA24D9"};
    u8 xiaoxiao[] = {"C63B0685"};
    u8 xiongyuqiu[] = {"E7B90685"};
		
    if(strstr((char *)num,(char *)zhouzhiwei))
    {
        printf("周智维开门\r\n");
        return 1;
    }
    else if(strstr((char *)num,(char *)yinzhibo))
    {
        printf("殷智博开门\r\n");
        return 2;
    }
    else if(strstr((char *)num,(char *)xiaoxiao))
    {
        printf("肖晓开门\r\n");
        return 3;
    }
    else if(strstr((char *)num,(char *)xiongyuqiu))
    {
        printf("熊宇秋开门\r\n");
        return 4;
    }
